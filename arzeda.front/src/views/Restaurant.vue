                ...
                <v-chip-group>
                  ...
                  <v-menu offset-y>
                    <template v-slot:activator="{ on, attrs }">
                        <v-chip class="ma-2" v-bind="attrs"
                        v-on="on">i</v-chip>
                    </template>
                    <v-card class="pa-2 grey--text">
                        <h3 class="black--text">{{ restaurant.name }}</h3>
                        <p>
                            <span>{{ restaurant.address }}</span> &middot; <span>Режим работы: {{ restaurant.timeWork }}</span>
                        </p>
                        <p>
                            <span v-for="category in restaurant.categories" :key="category.id">{{ category.name }}</span>
                        </p>
                    </v-card>
                </v-menu>
                </v-chip-group>
...
export default class RestaurantView extends Vue {
    ...
    async mounted() {
        const response = await this.$axios.get("/api/restaurant/" + this.$route.params.id);
